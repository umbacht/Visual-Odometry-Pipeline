...
% Find new candidates (Harris corners) in current image
%
% Input:
% - Current Image
% - Current State
...
%
% Output:
% - Current state with new candidates
...
if isfield(parameter, 'distortion_frame')
	...
	harris_features = harris(image_croped, parameter.harris_patch_size, ...
        parameter.harris_kappa);
    harris_features = padarray(harris_features, ...
        [parameter.distortion_frame(2),parameter.distortion_frame(1)],0);
else
	harris_features = harris(image_crt, parameter.harris_patch_size, ...
        parameter.harris_kappa);
end
...
% Selecting KeyPoints 
keypoints = selectKeypoints(harris_features, parameter.num_keypoints, ...
...
min_distances = pdist2(database_keypoints, keypoints, ...
